...
function showTooltip(e, info) {
    ...
    if(!info) { // Kosong
        ...
        tooltip.style.backgroundColor = '#4CAF50'; // hijau
    ...
    tooltip.textContent = `Lapak #${num}: ${nama} (${status})`;

    if(!status && !bayar) { // Kosong
        tooltip.style.backgroundColor = '#4CAF50';
    } else if(!bayar) { // Belum bayar
...
// Warna background lapak juga bisa otomatis:
