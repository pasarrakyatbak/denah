...
// Fungsi tambah simbol jembatan sungai di samping lapak tertentu
function insertJembatanSungai(parent, targetNum) {
    const children = Array.from(parent.children);
    const idx = children.findIndex(c => c.textContent == String(targetNum));
    if (idx === -1) return;

    const jembatan = document.createElement("div");
    jembatan.className = "jembatan-sungai";
    jembatan.innerHTML = "🌉<br><span>Sungai</span>";

    parent.insertBefore(jembatan, children[idx + 1]);
}

// Tambah jembatan sungai di samping lapak 62
insertJembatanSungai(leftSide, 62);
