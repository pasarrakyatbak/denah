...
function insertTiang(rumputWrapper, lapakContainer, a, b = null) {
    ...
    if (b === null) {
        const targetLapak = lapaks.find(c => c.textContent.trim() == a);
        if (!targetLapak) return;
        targetRect = targetLapak.getBoundingClientRect();
    } else {
        ...
        targetRect = {
            left: (rectA.left + rectB.left + rectB.width) / 2,
            width: 0
        };
    }
    ...
    tiang.textContent = "TIANG DEBUG"; // 🔴 tulisan jelas

    const offsetLeft = targetRect.left - rumputRect.left + targetRect.width / 2;
    tiang.style.left = `${offsetLeft}px`;
...
insertTiang(rumputRight, rightSide, 20, 201);
insertTiang(rumputRight, rightSide, 108, 109);
insertTiang(rumputRight, rightSide, 124, 125);
insertTiang(rumputRight, rightSide,137, 138);
