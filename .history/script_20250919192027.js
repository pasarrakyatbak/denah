...
function insertTiang(parent, a, b = null) {
    const children = Array.from(parent.children);

    if (b === null) {
        // Mode 1: tiang tepat di atas lapak a
        const target = children.find(c => c.textContent == a);
        if (!target) return;
        ...
        // Mode 2: tiang di antara lapak a dan b
        const idxA = children.findIndex(c => c.textContent == a);
        const idxB = children.findIndex(c => c.textContent == b);
        if (idxA === -1 || idxB === -1) return;
