...
function insertTiang(rumputWrapper, lapakContainer, a, b = null) {
    const lapaks = Array.from(lapakContainer.children);
    let targetLapak;

    if (b === null) {
        // Mode 1: tepat di atas lapak a
        targetLapak = lapaks.find(c => c.textContent.trim() == a);
    } else {
        // Mode 2: di antara lapak a dan b
        const lapakA = lapaks.find(c => c.textContent.trim() == a);
        const lapakB = lapaks.find(c => c.textContent.trim() == b);
        if (!lapakA || !lapakB) return;

        // cari titik tengah antara A dan B
        const rectA = lapakA.getBoundingClientRect();
        const rectB = lapakB.getBoundingClientRect();
        const centerX = (rectA.left + rectB.left + rectB.width) / 2;

        targetLapak = { getBoundingClientRect: () => ({ left: centerX, width: 0 }) };
    }

    if (!targetLapak) return;

    const rect = targetLapak.getBoundingClientRect();
    const rumputRect = rumputWrapper.getBoundingClientRect();

    const tiang = document.createElement('div');
    tiang.className = 'tiang-container';
    tiang.innerHTML = `📡 <span class="tiang-label">WiFi</span> 💡 <span class="tiang-label">Lampu</span>`;

    // posisikan relatif ke rumput
    tiang.style.position = "absolute";
    tiang.style.top = "-40px"; // naik di atas rumput
    tiang.style.left = (rect.left - rumputRect.left + rect.width / 2) + "px";
    tiang.style.transform = "translateX(-50%)";
    tiang.style.zIndex = "1"; // di bawah lapak

    rumputWrapper.appendChild(tiang);
}

// Ambil elemen rumput & container lapak kanan
const rumputRight = document.querySelector('.rumput-wrapper.right');
