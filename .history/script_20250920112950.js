...
fetchLapakData().then(data => {
    data.forEach(lapak => {
        lapakDataMap[lapak.no] = lapak;
    });

    // Update tooltip dan modal
    document.querySelectorAll('.lapak').forEach(el => {
        const num = el.textContent;
        const lapakInfo = lapakDataMap[num];

        // Tooltip
        el.addEventListener('mouseenter', e => {
            if (lapakInfo) {
                showTooltip(e, `Lapak #${num}: ${lapakInfo.nama}`);
            } else {
        ...
        el.addEventListener('click', () => {
            if (lapakPhotos[num]) {
                ...
                caption.textContent = lapakInfo ? `${lapakInfo.nama}` : 'Lapak #' + num;
            } else {
