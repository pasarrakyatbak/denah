...
function showTooltip(e, info) {
    ...
    let yTop = targetRect.top - containerRect.top + container.scrollTop - 8; // posisi atas
    let yBottom = targetRect.bottom - containerRect.top + container.scrollTop + 8; // posisi bawah

    // Default: taruh di atas lapak
    tooltip.style.left = `${x - (tooltip.offsetWidth / 2)}px`;
    tooltip.style.top = `${yTop - tooltip.offsetHeight}px`;
    tooltip.style.opacity = 1;

    // Kalau tooltip kepotong ke atas (kurang dari 0), pindahkan ke bawah
    if (tooltip.offsetTop < container.scrollTop) {
        tooltip.style.top = `${yBottom}px`;

        // Geser arrow ke atas (biar nyambung ke bawah lapak)
