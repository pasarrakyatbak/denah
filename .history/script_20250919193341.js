...
function insertTiang(rumputWrapper, lapakElement) {
    const tiang = document.createElement('div');
    tiang.className = 'tiang-container';
    tiang.innerHTML = `📡 <span>WiFi</span> 💡 <span>Lampu</span>`;

    // Posisi tiang mengikuti posisi lapak
    const lapakRect = lapakElement.getBoundingClientRect();
    const parentRect = rumputWrapper.getBoundingClientRect();

    const offsetLeft = lapakRect.left - parentRect.left + (lapakRect.width / 2);
...
// ambil rumput-wrapper kanan
const rumputRight = document.querySelector('.rumput-wrapper.right');
// ambil lapak 108
const lapak108 = document.querySelector('.lapak:nth-child(108)');

// taruh tiang di belakang rumput, sejajar dengan lapak 108
insertTiang(rumputRight, lapak108);
