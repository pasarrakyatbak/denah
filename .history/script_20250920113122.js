...
async function fetchLapakData() {
    try {
        ...
        if (json.success) {
            return json.data; // array lapak
        } else {
            console.error('API gagal', json);
            return [];
        }
    } catch (err) {
...
const lapakDataMap = {};
// Ambil elemen modal dan tooltip
...
// Setelah data lapak diambil, update tooltip dan modal
// Setelah data lapak tersedia, update tooltip dan modal
fetchLapakData().then(data => {
    ...
    document.querySelectorAll('.lapak').forEach(el => {
        ...
        // Tooltip menampilkan nama + jenis jualan + status bayar
        el.addEventListener('mouseenter', e => {
            if (info) {
                showTooltip(e, `Lapak #${num}: ${info.nama} (${info.status}${info.bayar ? ', Lunas' : ', Belum bayar'})`);
            } else {
                showTooltip(e, 'Lapak #' + num);
            }
        });
        ...
        // Modal menampilkan foto + nama + jualan
...
document.querySelectorAll('.jalan-gerbang').forEach(el => {
    el.addEventListener('mouseenter', e => showTooltip(e, 'Jalan'));
    el.addEventListener('mousemove', e => showTooltip(e, 'Jalan'));
    el.addEventListener('mouseleave', hideTooltip);
});
