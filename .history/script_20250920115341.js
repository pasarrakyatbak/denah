...
function showTooltip(e, info) {
    ...
    // Estimasi ukuran tooltip
    const tooltipWidth = tooltip.offsetWidth || 160;
    const tooltipHeight = tooltip.offsetHeight || 40;

    // Koordinat relatif terhadap container (pakai clientX/clientY)
    let x = e.clientX - containerRect.left + container.scrollLeft + 10;
    let y = e.clientY - containerRect.top + container.scrollTop + 10;

    // Periksa overflow kanan
    if (x + tooltipWidth > container.scrollLeft + container.offsetWidth) {
        x = container.scrollLeft + container.offsetWidth - tooltipWidth - 10;
    }

    // Periksa overflow bawah
    if (y + tooltipHeight > container.scrollTop + container.offsetHeight) {
        y = container.scrollTop + container.offsetHeight - tooltipHeight - 10;
    }
    ...
    // Isi teks tooltip
    if (!info || !info.nama) {
        tooltip.textContent = 'Lapak kosong';
        tooltip.style.backgroundColor = '#4CAF50';
        return;
    }

    const { num, nama, status, bayar } = info;
    tooltip.textContent = `Lapak #${num}: ${nama} (${status || 'Terisi'})`;
    tooltip.style.backgroundColor = bayar ? '#2196F3' : '#F44336';
}
    ...
    document.querySelectorAll('.lapak').forEach(el => {
        ...
        el.addEventListener('click', () => {
            if (lapakPhotos[num]) {
                ...
                caption.textContent = info ? `${info.nama} (${info.status}${info.bayar ? ', Lunas' : ', Belum bayar'})` : 'Lapak #' + num;
            } else alert('Foto belum tersedia untuk lapak #' + num);
        });
